...
The repository workspace at `https://jules.google.com/repo/github/balajirajput96/github-mcp-server-/overview` shows the initial maintenance session as **Paused**. It has not created a confirmed plan, code change, branch, pull request, merge, or release. The repository view includes an explicit **Scheduled** filter and a new-task composer. The daily Manus progress schedule is active at 09:00 Asia/Kolkata; the native Jules recurring task has not yet been created because the task-mode selector needs further safe UI verification.
...
The companion Manus schedule, **Daily GitHub owner-repo and Jules update**, is verified **active**. It runs daily at 09:00 Asia/Kolkata in full-auto mode and is constrained to reporting, evidence-based triage, and the next safe action; it is explicitly prohibited from merging pull requests, deleting repositories or branches, modifying secrets or branch protections, publishing releases, or making billing/permission changes.
