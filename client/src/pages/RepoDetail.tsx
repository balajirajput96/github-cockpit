...
function WorkflowDrilldown() {
  ...
  const [rerunGuidanceFor, setRerunGuidanceFor] = useState<string | null>(null);
    ...
    <div className="detail-content-grid">
      <section className="detail-main-column">
        <div className="detail-panel">
          ...
          <div className="workflow-list">{runs.map((run) => <div className="workflow-row" key={`${run.branch}-${run.name}`}><div className={`workflow-node node-${run.state}`}><Workflow size={15} /></div><div className="workflow-copy"><div><strong>{run.name}</strong><span className="branch-tag"><GitBranch size={12} />{run.branch}</span></div><span>{run.commit} · {run.time}</span></div><div className="workflow-duration"><Clock3 size={13} />{run.duration}</div><StatePill state={run.state} /><button className="button button-ghost" onClick={() => { setRerunGuidanceFor(run.name); toast("Review-safe snapshot", { description: "Open GitHub Actions and confirm the branch before any rerun." }); }} aria-label={`Open rerun guidance for ${run.name}`}>Rerun guidance <ArrowUpRight size={14} /></button></div>)}</div>
          {rerunGuidanceFor && <div className="inline-notice"><CircleAlert size={14} /> <strong>{rerunGuidanceFor}</strong> can only be rerun from GitHub Actions after a reviewer confirms the branch. This cockpit does not trigger workflow writes.</div>}
        </div>
